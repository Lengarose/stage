...
CREATE TABLE IF NOT EXISTS players (
  ...
  game_day_tile_backgrounds JSON,
  shirt_number          INT,
...
SET @sql=(SELECT IF(EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name=@t AND column_name='game_day_tile_backgrounds'),'SELECT 1','ALTER TABLE players ADD COLUMN game_day_tile_backgrounds JSON NULL')); PREPARE s FROM @sql; EXECUTE s; DEALLOCATE PREPARE s;
SET @t='clubs';
