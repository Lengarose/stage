...
function loadMessageDeliveryServiceWithDbMock(executesql) {
  ...
  return { service: require(servicePath), broadcasts };
}
...
test('deliverContractOfferMessage creates one inbox message and one related notification', async () => {
  ...
  assert.equal(inboxInserts[0].params.at(-1), 'contract-1');
  assert.deepEqual(notificationLookup.params, ['player@example.test', 'contract_offer', 'contract-1']);
  assert.equal(notificationInserts[0].params.at(-1), 'contract-1');
  assert.equal(broadcasts.length, 1);
  assert.equal(broadcasts[0].related_id, 'contract-1');
});
