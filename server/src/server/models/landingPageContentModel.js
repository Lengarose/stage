...
class LandingPageContent {
  ...
  static normalizeRow(row = {}) {
    ...
    if (typeof next.stats_json === 'string' && next.stats_json) {
      try { next.stats_json = JSON.parse(next.stats_json); } catch { next.stats_json = null; }
    }
    return next;
  }
  ...
  create() {
    ...
    return EXECUTESQL(
      ...
      [
        ...
        typeof this.stats_json === 'object' ? JSON.stringify(this.stats_json) : (this.stats_json || null),
