...
class Comment {
  ...
  create() {
    ...
    const sql = `INSERT INTO comments (id, post_id, author_email, content) VALUES (?,?,?,?)`;
    return EXECUTESQL(sql, [this.id, this.post_id, this.author_email, this.content]);
  }

  update(id) {
    const sql = `UPDATE comments SET post_id=?, author_email=?, content=? WHERE id=?`;
    return EXECUTESQL(sql, [this.post_id, this.author_email, this.content, id]);
  }
