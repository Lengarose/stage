...
/**
 * Post-login home page content. Edited by admins via HomePageEditor.jsx.
 * Kept separate from `landing_page_contents` (pre-login marketing site) so the two pages
 * can be edited and themed independently.
 */
class HomePageContent {
  ...
  static normalizeRow(row = {}) {
    ...
    if (typeof next.faq_items === 'string') {
      try { next.faq_items = JSON.parse(next.faq_items || '[]'); } catch { next.faq_items = []; }
    }
  ...
  selectAll(limit = 50) {
    return EXECUTESQL(
      'SELECT * FROM home_page_contents ORDER BY updated_date DESC LIMIT ?',
      [Number(limit) || 50]
    ).then(rows => rows.map(r => HomePageContent.normalizeRow(r)));
  }

  selectOne(id) {
    return EXECUTESQL('SELECT * FROM home_page_contents WHERE id = ? LIMIT 1', [id])
      .then(rows => rows.map(r => HomePageContent.normalizeRow(r)));
  }
  ...
  create() {
    ...
    return EXECUTESQL(
      `INSERT INTO home_page_contents (
        id, hero_title, hero_subtitle, hero_description, hero_image_url, hero_image_position, hero_image_zoom,
        hero_cta_1_label, hero_cta_1_url, hero_cta_2_label, hero_cta_2_url, hero_cta_3_label, hero_cta_3_url,
        section1_title, section1_text, section1_image_url, section1_image_position, section1_image_zoom,
        section2_title, section2_text, section2_image_url, section2_image_position, section2_image_zoom,
        section3_title, section3_text, section3_image_url, section3_image_position, section3_image_zoom,
        faq_items, contact_email, footer_tagline
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        this.id, this.hero_title, this.hero_subtitle, this.hero_description, this.hero_image_url, this.hero_image_position, this.hero_image_zoom,
        ...
        this.section1_title, this.section1_text, this.section1_image_url, this.section1_image_position, this.section1_image_zoom,
        this.section2_title, this.section2_text, this.section2_image_url, this.section2_image_position, this.section2_image_zoom,
        this.section3_title, this.section3_text, this.section3_image_url, this.section3_image_position, this.section3_image_zoom,
  ...
  update(id) {
    return EXECUTESQL(
      `UPDATE home_page_contents SET
        hero_title=?, hero_subtitle=?, hero_description=?, hero_image_url=?, hero_image_position=?, hero_image_zoom=?,
        hero_cta_1_label=?, hero_cta_1_url=?, hero_cta_2_label=?, hero_cta_2_url=?, hero_cta_3_label=?, hero_cta_3_url=?,
        section1_title=?, section1_text=?, section1_image_url=?, section1_image_position=?, section1_image_zoom=?,
        section2_title=?, section2_text=?, section2_image_url=?, section2_image_position=?, section2_image_zoom=?,
        section3_title=?, section3_text=?, section3_image_url=?, section3_image_position=?, section3_image_zoom=?,
      ...
      WHERE id=?`,
      [
        this.hero_title, this.hero_subtitle, this.hero_description, this.hero_image_url, this.hero_image_position, this.hero_image_zoom,
        this.hero_cta_1_label, this.hero_cta_1_url, this.hero_cta_2_label, this.hero_cta_2_url, this.hero_cta_3_label, this.hero_cta_3_url,
        this.section1_title, this.section1_text, this.section1_image_url, this.section1_image_position, this.section1_image_zoom,
        this.section2_title, this.section2_text, this.section2_image_url, this.section2_image_position, this.section2_image_zoom,
        this.section3_title, this.section3_text, this.section3_image_url, this.section3_image_position, this.section3_image_zoom,
  ...
  delete(id) {
    return EXECUTESQL('DELETE FROM home_page_contents WHERE id = ?', [id]);
  }
}
