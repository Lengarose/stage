...
function loadSocialRoutersWithMocks(executesql, notifications, broadcasts) {
  ...
  for (const modulePath of [postControllerPath, commentControllerPath, postModelPath, commentModelPath, dbPath, deliveryPath, mentionPath, socketPath]) {
...
test('POST /comments resolves gamertag server-side and notifies owner and mentioned player', async () => {
  ...
  const { commentRouter } = loadSocialRoutersWithMocks(async (sql, params = []) => {
    ...
    if (/FROM players WHERE LOWER\(email\) = LOWER\(\?\) LIMIT 1/.test(sql)) return [{ gamertag: 'ActorTag', avatar_url: '/actor.png' }];
    ...
    if (/UPDATE posts SET comments_count = COALESCE\(comments_count, 0\) \+ 1 WHERE id = \?/.test(sql)) return { affectedRows: 1 };
