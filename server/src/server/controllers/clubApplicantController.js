...
async function deliverApplicantContractOffer(contractId) {
  ...
  if (!contract || !recipientEmail) return;
  ...
  const body = [
    `${contract.club_name || 'A club'} has sent you a ${String(contract.contract_type || 'squad').replace(/_/g, ' ')} contract offer.`,
    '',
...
async function offerContract(req, res, isTrial) {
  try {
    ...
    await deliverApplicantContractOffer(contractId).catch(err => console.error('[applicant contract inbox]', err.message));
