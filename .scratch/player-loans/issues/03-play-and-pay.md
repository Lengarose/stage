...
- [ ] Playing club is the borrower; owner club cannot select (`LOAN_PLAYER_NOT_ELIGIBLE`); borrower can
- [ ] Squad: Club B shows LOAN badge; Club A shows On loan at B until the end date
- [ ] Profile current club is the playing club, with on-loan-from the owner; parent contract still listed with Club A
- [ ] Lineup persistence rejects a player whose playing club is not that club (server-side)
- [ ] Wage 30/70 (and 0/100, 100/0) debits the two clubs and credits the player for 100% of weekly salary
- [ ] Owner wage bill uses the parent share; borrower wage bill uses the loan share
- [ ] One club’s wage shortfall does not silently charge the other club; the loan stays `ACTIVE`
- [ ] Player-president on loan at B still presides over A
- [ ] Callers (squad list, salary job, lineup) use the player-loan module; they do not copy loan conditionals
