/**
 * payMonthlySalaries — scheduled monthly salary payout to all active contracted players.
 * Reads all active PlayerContracts with weekly_salary_stc > 0,
 * calculates weeks since last payment, pays accordingly.
 * Triggered by a monthly automation (end of month UTC).
 */
    ...
    for (const contract of salaryContracts) {
      try {
        ...
        await base44.asServiceRole.entities.STCTransaction.create({
          ...
          amount: actualAmount,
          type: 'salary',
          description: `Monthly salary payment (${weeksSincePaid} week${weeksSincePaid > 1 ? 's' : ''}) from ${club?.name || 'club'}`,
        ...
        await base44.asServiceRole.entities.Notification.create({
          ...
          type: 'achievement',
          title: `💰 Salary paid: +${actualAmount.toLocaleString()} STC`,
          body: `${club?.name || 'Your club'} paid you ${weeksSincePaid} week${weeksSincePaid > 1 ? 's' : ''} of salary.`,
          link: '/lifestyle',
          read: false,
        });
